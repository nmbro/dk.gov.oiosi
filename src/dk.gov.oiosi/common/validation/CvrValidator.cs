...
namespace dk.gov.oiosi.common.validation {
    ...
    /// <summary>
    /// Class to validate a cvr string
    /// </summary>
    public class CvrValidator {

        /// <summary>
        /// Validates the cvr-string and returns eventual failures
        /// </summary>
        ...
        public static bool IsValid(string Value, string ValueName, bool Required, 
            ...
            if ( !ValUtil.IsEmpty(Value)) {
                if (!Regex.IsMatch(Value, @"(\d)")) {
                    DataValidationFailure.AddFailure(InvalidCvrFailure.Message(Value), ValueName, typeof(IdentifierCvr), ref Failures);
                    Valid = false;
                }
                if (Valid)
                    if (Value.Length<8){
                ...
                if (Required) {
                    DataValidationFailure.AddFailure(
                        RequiredFieldFailure.Message(ValueName), ValueName, typeof(IdentifierCvr), ref Failures);
                    Valid = false;
                }
