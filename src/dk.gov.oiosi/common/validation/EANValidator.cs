...
namespace dk.gov.oiosi.common.validation {
    /// <summary>
    /// Class the validate Ean numbers
    /// </summary>
    public class EanValidator {
        ...
        public static bool IsValid(string Value, string ValueName, bool Required, 
            ...
            if (!ValUtil.IsEmpty(Value)) {
                if (!Regex.IsMatch(Value, @"(57\d)")) {
                    DataValidationFailure.AddFailure(InvalidEanFailure.Message(Value), ValueName, typeof(IdentifierCvr), ref Failures);
                    Valid = false;
                }
                if (Valid)
                    if (Value.Length != 13) {
                ...
                if (Required) {
                    DataValidationFailure.AddFailure(
                        RequiredFieldFailure.Message(ValueName), ValueName, typeof(IdentifierEan), ref Failures);
                    Valid = false;
                }
