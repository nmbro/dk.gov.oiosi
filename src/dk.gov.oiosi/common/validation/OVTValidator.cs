...
namespace dk.gov.oiosi.common.validation {
    /// <summary>
    /// Class the validate OVT numbers
    /// </summary>
    public class OvtValidator {

        /// <summary>
        /// Validates the OVT-string
        /// </summary>
        ...
        public static bool IsValid(string Value, string ValueName, bool Required, 
            ...
            if (!ValUtil.IsEmpty(Value)) {
                try {
                    IIdentifier identifier = new IdentifierOvt(Value);
                } catch {
                    Valid = false;
                }
                if (!Valid) {
                    try {
                        ...
                        DataValidationFailure.AddFailure(InvalidOvtFailure.Message(Value), ValueName, typeof(IdentifierOvt), ref Failures);
                        Valid = false;
                    }
                }
            } else 
                ...
                if (Required) {
                    DataValidationFailure.AddFailure(
                        RequiredFieldFailure.Message(ValueName), ValueName, typeof(IdentifierOvt), ref Failures);
                    Valid = false;
                }
